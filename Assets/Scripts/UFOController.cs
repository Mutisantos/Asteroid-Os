	...
	void Update () {
		if(GameManager.instance.getLevel() >= firstLevelforUFOs && noUFOSLeft()){
			...
			if(respawnTimer <= 0 ){
				...
				UFO newUfo = Instantiate(ufoPrefabs[Random.Range(0,ufoPrefabs.Length-1)],waypoint.getBeginPosition(),direction);
				newUfo.setWaypointPositions(waypoint.getBeginPosition(), waypoint.getEndPosition());
